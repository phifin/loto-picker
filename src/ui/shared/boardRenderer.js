...
export class BoardRenderer {
  ...
  renderMultipleBoards(boards, multiBoardMgr, onCellClick) {
    ...
    boards.forEach((boardData, index) => {
      ...
      if (boards.length > 1) {
        ...
        prevBtn.onclick = () =>
          this.navigateToBoard(index - 1, boards, multiBoardMgr, onCellClick);
  ...
  resetMarkedCells() {
    this.boardsContainer
      ...
      .forEach((el) =>
        el.classList.remove("marked", "row-complete", "flash")
      );
  }
  ...
  navigateToBoard(targetIndex, boards, multiBoardMgr, onCellClick) {
    ...
    if (this.currentSwiper) {
      ...
      const targetBoard = this.boardsContainer.querySelector(
        `[data-board-id="${boards[targetIndex].id}"]`
      );
      if (targetBoard) {
  ...
  getRowCellsForBoard(boardId, rowIndex) {
    const boardEl = this.boardsContainer.querySelector(
  ...
  getCellElement(boardId, number) {
    const cells = this.boardsContainer.querySelectorAll(
      `.cell.num[data-board-id="${boardId}"]`
    );
